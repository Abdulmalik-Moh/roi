...
const API_BASE_URL = 'http://localhost:3000/api'; // Simple fix!
...
class AuthService {
    ...
    async handleContactForm() {
        ...
        if (!nameInput || !emailInput || !messageInput || !contactBtn) {
            ...
            this.showContactMessage('Contact form not properly loaded. Please refresh the page.', 'error');
            return;
        }
    ...
    showUserProfile() {
        const modalContent = document.querySelector('.modal-content');
        if (!this.isLoggedIn() || !modalContent) return;
            ...
            <div class="modern-profile">
                <div class="profile-card">
                    <div class="profile-avatar">
                        <div class="avatar-circle">
                            <span>${this.user.name.charAt(0)}</span>
                        </div>
                        <div class="online-status"></div>
                    </div>
                    <div class="profile-info">
                        <h3 class="profile-name">${this.user.name}</h3>
                ...
                <div class="recent-orders">
                    <div class="section-header">
                        ...
                        <button class="view-all-btn" id="viewAllOrdersBtn">View All</button>
                    </div>
...
class DataSync {
    ...
    async syncProducts(filters = {}) {
        ...
        try {
            ...
            const url = new URL(`${this.BASE_URL}/products`, window.location.origin);
            
            if (filters.category && filters.category !== 'all') {
                url.searchParams.append('category', filters.category);
            }
            if (filters.skinType && filters.skinType !== 'all') {
                url.searchParams.append('skinType', filters.skinType);
            }
            if (filters.priceRange && filters.priceRange !== 'all') {
                url.searchParams.append('priceRange', filters.priceRange);
            }
            if (filters.search) {
            ...
            const data = await response.json();
            console.log('✅ Received:', data.data?.length || 0, 'products');
            ...
            return this.getLocalProducts(); // Fallback to local
        }
    }
...
function attachProductClickListeners() {
    document.querySelectorAll('.product-card').forEach(card => {
        card.addEventListener('click', function(e) {
            ...
            console.log('Product card clicked - detail view coming soon!');
        });
    });
}
...
async function displayFeaturedProducts() {
    ...
    try {
        ...
        console.log('📡 Fetching from /api/products/featured');
        ...
        if (response.ok) {
            ...
            if (data.success && Array.isArray(data.data) && data.data.length > 0) {
                ...
                console.log(`✅ Found ${featuredProducts.length} featured products from API`);
            } else {
                console.log('⚠️ API returned empty or no data, using local fallback');
                featuredProducts = products.slice(0, 4);
            }
        } else {
            console.log(`❌ API error ${response.status}, using local fallback`);
            featuredProducts = products.slice(0, 4);
        }
