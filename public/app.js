...
const API_BASE_URL = 'https://roi-beauty-api.onrender.com/api';
...
class DataSync {
    constructor() {
        ...
        console.log('🌐 DataSync: Using API at', this.BASE_URL);
    }
    ...
    async syncProducts(filters = {}) {
        ...
        try {
            ...
            const url = queryString ? `${this.BASE_URL}/products?${queryString}` : `${this.BASE_URL}/products`;
            ...
            if (!response.ok) {
                console.warn('⚠️ API response not OK, using local products');
            ...
            if (data.success && Array.isArray(data.data)) {
                ...
                console.warn('⚠️ Invalid API response format, using local products');
                return this.getLocalProducts(filters);
            }
    ...
    async syncFeaturedProducts() {
        ...
        try {
            ...
            if (!response.ok) {
                console.warn('⚠️ Featured API failed, using local fallback');
            ...
            if (data.success && Array.isArray(data.data) && data.data.length > 0) {
                ...
                console.warn('⚠️ No featured products in API response, using local fallback');
                return this.getLocalProducts().slice(0, 4);
            }
